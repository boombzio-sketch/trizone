...
router.post('/register', async (req, res) => {
  ...
  const exists = await db.prepare('SELECT id FROM users WHERE nickname = ?').get(nickname);
...
router.post('/login', async (req, res) => {
  ...
  const user = await db.prepare('SELECT * FROM users WHERE nickname = ?').get(nickname);
